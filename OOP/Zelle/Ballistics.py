...
class Projectile:
    ...
    def update(self, time, maxheight):
        if self.getY() > maxheight:
            maxheight = self.getY()
...
def main():
    ...
    while p.getY() >= 0:
        p.update(time, maxheight)
        maxheight = p.update(time, maxheight)
    print("\nDistance travelled: {0:0.1f} meters.".format(p.getX()), 'Highest:', maxheight, "meters")
